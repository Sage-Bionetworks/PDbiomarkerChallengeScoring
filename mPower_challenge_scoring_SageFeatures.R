...
# replace rest and walk features here
# leave walkmetadata and restmetadata as is
...
# Prepare Data
walkvariables<-names(trainingfeatures$walk)
walkvariables<-walkvariables[!tolower(walkvariables) %in% c("recordid","healthcode")]
trainingwalk<-merge.data.frame(trainingfeatures$walkmetadata, trainingfeatures$walk, by=c("healthCode","recordId"), all.x=T, sort=F)
...
model_list_nophoneapp <- caretList(
  ...
  metric = "ROC",
  methodList=c("glmnet", "rf", "svmLinear", "knn", "nnet"),
...
# Prepare Data
restvariables<-names(trainingfeatures$rest)
restvariables<-restvariables[!tolower(restvariables) %in% c("recordid","healthcode")]
trainingrest<-merge.data.frame(trainingfeatures$restmetadata, trainingfeatures$rest, by=c("healthCode","recordId"), all.x=T, sort=F)
...
testfeatureids<-c(rest="syn9955511", walk="syn9926958", walkmetadata="syn10138365", restmetadata="syn10140543")
...
# Prepare Data
testwalk<-merge.data.frame(testfeatures$walkmetadata, testfeatures$walk, by=c("healthCode","recordId"), all.x=T, sort=F)
...
# Prepare Data
testrest<-merge.data.frame(testfeatures$restmetadata, testfeatures$rest, by=c("healthCode","recordId"), all.x=T, sort=F)
